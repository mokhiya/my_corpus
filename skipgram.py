...
def checkWord(word):
    ...
    for i in range(10): #ishlash vaqtini kamaytirish uchun qo'yildi
        if str(i) in word:
            ok = False
    return ok
def analyze():
...
def predict(text):
    ...
    if find_token in words:
        ...
        for word in dictionary:
            words[ind] = word
            tmp = countPairs(prev_word, word) / (dictionary[prev_word])

            if ind + 1 < len(words):
                tmp = (tmp + countPairs(word, words[ind + 1]) / dictionary[word]) / 2
            if tmp > prob:
                prob = tmp
                ans = word
...
print()
res = predict("men | bordim")[1]
res.sort(reverse=True)
for v in res[:10]:
    print("%-20s %.16f" % (v[1], v[0]))

print()
res = predict("o'zbek tili davlat |")[1]
res.sort(reverse=True)
for v in res[:25]:
    print("%-40s %.16f" % (v[1], v[0]))

print()
res = predict("barcha tumanlarda | energiyasi uzilib qoldi")[1]
res.sort(reverse=True)
for v in res[:10]:
    print("%-60s %.16f" % (v[1], v[0]))
